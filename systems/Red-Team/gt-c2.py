"""
C2 / post-exploitation GT simulator (lab only)
...
# ====== CONFIG ======
USERNAME = "test"       # change to real lab user if needed (e.g. msfadmin)
PASSWORD = "test"       # change to real lab password
WAIT_SECONDS = 60
GROUND_TRUTH = "./ground/c2.csv"
LABEL = "c2"
# ====================
...
def run_ssh_session(target, username, password, src_ip):
    ...
    try:
        ...
        # Log several GT rows for this session (port 22)
        for _ in range(5):
            log_event(src_ip, target, 22, LABEL)
            time.sleep(0.05)

        commands = ["pwd", "ls", "ls -la", "whoami", "uname -a"]
        for cmd in commands:
            print(f"[*] Running: {cmd}")
            ...
            if error:
                print("ERR:", error)
            # log traffic after each command
            log_event(src_ip, target, 22, LABEL)
            time.sleep(1)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 gt-c2.py <IP>")
        print("Example: python3 gt-c2.py 192.168.64.131")
        sys.exit(1)
